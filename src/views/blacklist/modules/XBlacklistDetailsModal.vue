    ...
    <a-spin :spinning="confirmLoading">
      <a-form layout="inline" class="warn-details-form">
        <a-row :gutter="0">
          ...
          <a-col :span="24">
            <div class="info-row">
              <span class="name">{{ xblacklist.customerName }}</span>
              <!-- <span class="type">{{ xblacklist.customerType | typeFormat(customerType) }}</span> -->
              <span class="status">{{ xblacklist.status | typeFormat(warnStatus) }}</span>
            </div>
          </a-col>
          <a-col :span="10">
            <a-form-item label="门店编号：">
              <span>{{ xblacklist.code }}</span>
            </a-form-item>
          </a-col>
          <a-col :span="10">
            <a-form-item label="所属品牌公司：">
              <span>{{ xblacklist.customerDepartment| distFormat('brand_types') }}</span>
            </a-form-item>
          </a-col>
          <!-- <a-col :span="10">
            <a-form-item label="证件类型：">
              <span>{{ xblacklist.zjlx | typeFormat(zjType) }}</span>
            </a-form-item>
          </a-col> -->
          <a-col :span="10">
            <a-form-item label="统一社会信用代码：">
              <span>{{ xblacklist.zjhm }}</span>
            </a-form-item>
          </a-col>
          <!-- 黑名单信息 -->
          <a-col :span="24">
            <div class="row-line"></div>
            <div class="row-header">黑名单信息</div>
          </a-col>
          <a-col :span="24">
            <a-form-item label="名单类型">
              <a-select v-if="isEdit === 1" v-model="xblacklist.type" placeholder="请选择名单类型" allowClear>
                <a-select-option v-for="(value, key, index) of hmdType" :key="index" :value="key">
                  {{ value }}
                </a-select-option>
              </a-select>
              <span v-else>{{ xblacklist.type | typeFormat(hmdType) }}</span>
            </a-form-item>
          </a-col>
          <a-col :span="24">
            <a-form-item label="录入原因">
              <a-textarea placeholder="请输入录入原因" v-if="isEdit === 1" v-model="xblacklist.remake" />
              <span v-else>{{ xblacklist.remake }}</span>
            </a-form-item>
          </a-col>
          <a-col :span="24" v-if="xblacklist.yjyy">
            <a-form-item label="黑名单解禁原因">
              <a-textarea placeholder="请输入预警解除原因" v-if="isEdit === 2" v-model="xblacklist.yjyy" />
              <span v-else>{{ xblacklist.yjyy }}</span>
            </a-form-item>
          </a-col>
          <!-- 驳回提交 -->
          <a-col :span="10" v-if="role === '1' && isEdit > 0 && userList.length > 0">
            <a-form-item label="协查用户">
              <a-select
                ...
                v-model="xblacklist.userThirdId"
                allowClear
              >
          ...
          <a-col :span="10" v-if="role === '2' && isEdit > 0 && userList.length > 0">
            <a-form-item label="审查用户">
              <a-select placeholder="请选择审查用户" style="width: 200px" v-model="xblacklist.userShenId" allowClear>
                <a-select-option v-for="(item, index) of userList" :key="index" :value="item.id">{{
                  item.realname
                }}</a-select-option>
              </a-select>
          ...
          <template v-if="ifBan">
            ...
            <a-col :span="12">
              <a-form-item label="处理结果：">
                <a-radio-group v-model="formData.rdjg">
                  ...
                  <a-radio v-if="!(xblacklist.status === 2 && role === '1')" :value="2"> 驳回修改 </a-radio>
            ...
            <a-col :span="12">
              <a-form-item
                ...
                v-if="!((xblacklist.status === 2 && role === '1') || xblacklist.status >= 4)"
              >
                <a-select style="width: 300px" placeholder="请选择审查用户" v-model="formData.nextUserId" allowClear>
                  <a-select-option v-for="(item, index) of userList" :key="index" :value="item.id">{{
                    item.realname
                  }}</a-select-option>
                </a-select>
...
import { customerType, warnStatus, zjType, hmdType, yjlcbz, roleTypes } from '@/utils/dictionary'
  ...
  data() {
    return {
      ...
      url: {
        ...
        queryById: '/credit/xBlacklist/queryById',
        checkWarning: '/credit/xBlacklist/checkBlack', //协查
        examinationWarning: '/credit/xBlacklist/examinationWarning', //审查
        approvalWarning: '/credit/xBlacklist/approvalWarning', //审批
      },
    }
  },
  created() {
  ...
  methods: {
    ...
    loadData() {
      ...
      getAction(this.url.queryById, param).then((res) => {
        if (res.success) {
          this.xblacklist = res.result.xblacklist
          this.xcommonAudits = res.result.xcommonAudits
          this.isHasXC = this.xblacklist.isHasXC === 1
          ...
          if (this.xblacklist.status === 0 && res.result.ifFrist) {
            this.isEdit = this.xblacklist.ifeffect === 1 ? 2 : 1
            this.getUserList()
          }
        } else {
    ...
    handleOk() {
      ...
      if (
        ...
        !((this.xblacklist.status === 2 && this.role === '1') || this.xblacklist.status >= 4)
      ) {
        this.$message.warning('请选择下一步处理人!')
        return
      }
      let url = this.url.checkWarning
      if (this.xblacklist.status === 3) {
        url = this.url.examinationWarning
      }
      if (this.xblacklist.status === 4) {
    ...
    handleEdit() {
      ...
      let param = Object.assign({}, this.xblacklist)
      param.status = 2
    ...
    getUserList() {
      //不需要选择 审查人
      if ((this.xblacklist.status === 2 && this.role === '1') || this.xblacklist.status >= 4) {
        return
      }
      let value = this.xblacklist.status + 1
      //驳回
      if (this.xblacklist.status === 0 && this.isEdit > 0) {
