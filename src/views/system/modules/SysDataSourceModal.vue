    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item :labelCol="labelCol"
                     :wrapperCol="wrapperCol"
                     label="数据库类型">
          <j-dict-select-tag placeholder="请选择数据库类型"
        ...
        <a-form-item :labelCol="labelCol"
                     :wrapperCol="wrapperCol"
                     label="驱动类">
          <a-input placeholder="请输入驱动类"
                   v-decorator="['dbDriver', validatorRules.dbDriver]" />
        </a-form-item>
        <a-form-item :labelCol="labelCol"
                     :wrapperCol="wrapperCol"
                     label="数据源地址">
          <a-input placeholder="请输入数据源地址"
...
export default {
  ...
  data() {
    return {
      ...
      dbUrlMap: {
        // MySQL 数据库
        1: { dbUrl: 'jdbc:mysql://127.0.0.1:3306/jjfinance?characterEncoding=UTF-8&useUnicode=true&useSSL=false' },
        // Oracle
      ...
      this.$nextTick(() => {
        this.form.validateFields(keys, (errors, values) => {
          if (!errors) {
            ...
            postAction('/online/cgreport/api/testConnection', fieldsValues)
              .then((res) => {
                if (res.success) {
