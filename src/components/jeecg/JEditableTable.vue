...
<template>
  <a-spin :spinning="loading">
      ...
      <a-col>
        ...
        <div v-if="actionButton"
          ...
          <a-button type="primary"
                    ...
                    :disabled="disabled">新增</a-button>
          <span class="gap"></span>
          <template v-if="selectedRowIds.length>0">
            <a-popconfirm :title="`确定要删除这 ${selectedRowIds.length} 项吗?`"
            ...
            <template v-if="showClearSelectButton">
              <a-button icon="delete"
                        @click="handleClickClearSelection">清空选择</a-button>
              <span class="gap"></span>
            </template>
          </template>
        </div>
    ...
    <slot name="actionButtonAfter"
          :target="getVM()" />

    <div :id="`${caseId}inputTable`"
         class="input-table">
      <!-- 渲染表头 -->
      <div class="thead"
           ref="thead">
        <div class="tr"
          ...
          <div v-if="rowSelection"
               class="td td-cb"
               :style="style.tdLeft">
            <!--:indeterminate="true"-->
            <a-checkbox :checked="getSelectAll"
          ...
          <template v-for="col in columns">
            <div v-show="col.type !== formTypes.hidden"
                 class="td"
                 :key="col.key"
      ...
      <div class="scroll-view"
        ...
        <div :id="`${caseId}tbody`"
          ...
          <!-- 扩展高度 -->
          <div class="tr-expand"
               :style="`height:${getExpandHeight}px; z-index:${loading?'11':'9'};`"></div>
          <!-- 无数据时显示 -->
                ...
                <div v-if="dragSort"
                  ...
                  <a-dropdown :trigger="['click']"
                    ...
                    <a-menu slot="overlay">
                      ...
                      <a-menu-divider />
                      <a-menu-item key="3"
                                   @click="_handleRowInsertDown(rowIndex)">插入一行</a-menu-item>
                    </a-menu>
                  </a-dropdown>
                </div>
                ...
                <div class="td"
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <template v-else-if="col.type === formTypes.select">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <a-select v-if="isEditRow(row, col)"
                                  :id="id"
                                  :key="i"
                                  v-bind="buildProps(row,col)"
                                  style="width: 100%;"
                        ...
                        <span v-else
                              class="j-td-span no-edit"
                              :class="{disabled: buildProps(row,col).disabled}"
                              @click.stop="handleEditRow(row, col)">{{ getSelectTranslateText(selectValues[id], row, col) }}</span>
                      </a-tooltip>
                    </template>
                    ...
                    <template v-else-if="col.type === formTypes.input_pop">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <j-input-pop v-if="isEditRow(row, col)"
                                     :id="id"
                                     :key="i"
                                     :width="300"
                                     :height="210"
                      ...
                      <template v-if="uploadValues[id] != null"
                        ...
                        <a-input :key="fileKey"
                          ...
                          <template slot="addonBefore"
                                    style="width: 30px">
                            <a-tooltip v-if="file.status==='uploading'"
                                       :title="`上传中(${Math.floor(file.percent)}%)`">
                              <a-icon type="loading" />
                            </a-tooltip>
                            <a-tooltip v-else-if="file.status==='done'"
                                       title="上传完成">
                          ...
                          <template v-if="col.allowDownload!==false || col.allowRemove!==false"
                                    slot="addonAfter"
                                    style="width: 30px">
                            <a-dropdown :trigger="['click']"
                              ...
                              <a-tooltip title="操作"
                                         :getPopupContainer="getParentContainer">
                                <a-icon v-if="file.status!=='uploading'"
                                        type="setting"
                                        style="cursor: pointer;" />
                              </a-tooltip>

                              <a-menu slot="overlay">
                                <a-menu-item v-if="col.allowDownload!==false"
                                             @click="handleClickDownloadFile(id)">
                                  <span>
                                    <a-icon type="download" />&nbsp;下载
                                  </span>
                                </a-menu-item>
                                <a-menu-item v-if="col.allowRemove!==false"
                    ...
                    <template v-else-if="col.type === formTypes.popup">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <j-popup v-if="isEditRow(row, col)"
                                 :id="id"
                                 :key="i"
                                 v-bind="buildProps(row,col)"
                                 :placeholder="replaceProps(col, col.placeholder)"
                    ...
                    <template v-else-if="col.type === formTypes.radio">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <a-radio-group :id="id"
                          ...
                          <a-radio v-for="(item, key) in col.options"
                                   :key="key"
                    ...
                    <template v-else-if="col.type === formTypes.list_multi">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        ...
                        <span v-else
                              class="j-td-span no-edit"
                              :class="{disabled: buildProps(row,col).disabled}"
                              @click="handleEditRow(row, col)">{{ getSelectTranslateText(multiSelectValues[id], row, col) }} </span>
                      </a-tooltip>
                    </template>
                    <!-- select多选 -end -->
                    ...
                    <template v-else-if="col.type === formTypes.sel_search">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        ...
                        <span v-else
                              class="j-td-span no-edit"
                              :class="{disabled: buildProps(row,col).disabled}"
                              @click="handleEditRow(row, col)">{{ getSelectTranslateText(searchSelectValues[id], row, col) }}</span>
                      </a-tooltip>
                    </template>
                    <!-- select搜索 -end -->
                    ...
                    <div v-else-if="col.type === formTypes.slot"
                         :key="i">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <slot :name="(col.slot || col.slotName) || col.key"
                              :index="rowIndex"
                              :text="slotValues[id]"
                              :value="slotValues[id]"
                              :column="col"
      ...
      <j-file-pop ref="filePop"
                  @ok="handleFileSuccess"></j-file-pop>
    </div>
  </a-spin>
</template>
